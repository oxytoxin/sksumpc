<tr class="hover:bg-green-200">
    ...
    @else
        @foreach ($this->month_pairs as $month_pair)
            <td class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
                <a href="{{ route('filament.app.pages.transactions-list', ['month' => $month_pair['next']['date']->month, 'year' => $month_pair['next']['date']->year, 'account_id' => $account->id, 'payment_mode' => 1]) }}" target="blank" class="inline-block w-full">
            ...
            <td class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
                <a href="{{ route('filament.app.pages.transactions-list', ['month' => $month_pair['current']['date']->month, 'year' => $month_pair['current']['date']->year, 'account_id' => $account->id, 'payment_mode' => 1]) }}" target="blank" class="inline-block w-full">
        ...
        @if ($child->tag == 'probable_loss')
            <tr>
                ...
                @foreach ($this->month_pairs as $month_pair)
                    <td class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
                        @if ($account_type->debit_operator == 1)
                            <a href="{{ route('filament.app.pages.transactions-list', ['month' => $month_pair['next']['index'], 'year' => $data['year'], 'account_id' => $account->id, 'payment_mode' => 1]) }}" target="blank" class="inline-block w-full">
