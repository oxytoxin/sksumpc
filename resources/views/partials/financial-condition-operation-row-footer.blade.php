<tr class="hover:bg-green-100">
    ...
    <td colspan="2" class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
        ...
        @else
            {{ renumber_format(sum_no_children_recursive($accounts, 'balance_forwarded_credit')) }}
        @endif
    </td>
    <td class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
        ...
        @else
            {{ format_percentage(sum_no_children_recursive($accounts, 'ending_balance'), sum_no_children_recursive($accounts, 'balance_forwarded_credit')) }}
        @endif
    </td>
    <td class="whitespace-nowrap border border-black px-2 text-right text-xs uppercase hover:bg-green-300">
    </td>
