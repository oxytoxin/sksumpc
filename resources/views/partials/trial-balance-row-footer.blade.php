@php
    $mso_account = findRecursive($this->accounts, fn($item) => $item->id == 55);
    $totals = [
        ...
        'total_crj_debit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_crj_debit') - $mso_account->total_crj_debit,
        'total_crj_credit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_crj_credit') - $mso_account->total_crj_credit,
        'total_cdj_debit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_cdj_debit') - $mso_account->total_cdj_debit,
        'total_cdj_credit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_cdj_credit') - $mso_account->total_cdj_credit,
        'total_jev_debit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_jev_debit') - $mso_account->total_jev_debit,
        'total_jev_credit' => sum_no_children_recursive($this->accounts->where('id', '!=', 55), 'total_jev_credit') - $mso_account->total_jev_credit,
        'total_crj_mso_debit' => $mso_account->total_crj_debit,
        'total_crj_mso_credit' => $mso_account->total_crj_credit,
        'total_cdj_mso_debit' => $mso_account->total_cdj_debit,
        'total_cdj_mso_credit' => $mso_account->total_cdj_credit,
        'total_jev_mso_debit' => $mso_account->total_jev_debit,
        'total_jev_mso_credit' => $mso_account->total_jev_credit,
...
<tr class="hover:bg-green-100">
    ...
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_crj_mso_debit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_crj_mso_credit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
    ...
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_cdj_mso_debit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_cdj_mso_credit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
    ...
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_jev_mso_debit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_jev_mso_credit']) }}
    </td>
    <td class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
...
<tr>
    ...
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_crj_mso_debit'] - $totals['total_crj_mso_credit']) }}
    </td>
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
    ...
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_cdj_mso_debit'] - $totals['total_cdj_mso_credit']) }}
    </td>
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
    ...
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
        {{ renumber_format($totals['total_jev_mso_debit'] - $totals['total_jev_mso_credit']) }}
    </td>
    <td colspan="2"
        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
