<tr class="hover:bg-green-200">
    <td @class([
        'border print:border-x print:border-y-0 border-black px-2 uppercase text-xs print:text-[8pt] whitespace-nowrap print:whitespace-normal',
    ...
    @if ($account->children_count > 0)
        @for ($i = 0; $i < 5; $i++)
            <td class="whitespace-nowrap border print:border-x print:border-y-0 border-black px-2 text-right text-xs uppercase hover:bg-green-300 print:whitespace-normal print:text-[8pt]">
            </td>
        @endfor
    @else
...
@if ($account->children_count > 0)
    @foreach ($account->children as $child)
        @if ($child->tag == 'probable_loss')
            <tr>
                <td class="whitespace-nowrap border print:border-x print:border-y-0 border-black px-2 pl-4 text-sm font-bold uppercase print:whitespace-normal print:text-[9pt]">
                    TOTAL {{ format_account_name_from_depth($account->fullname, $account->depth) }}
                </td>
                @if ($account_type->debit_operator == 1)
                    <td class="whitespace-nowrap border print:border-x print:border-y-0 border-black px-2 text-right font-bold text-xs uppercase hover:bg-green-300 print:whitespace-normal print:text-[8pt]">
                        <a href="{{ route('filament.app.pages.transactions-list', ['month' => $this->to_date->month, 'year' => $this->to_date->year, 'account_id' => $account->id, 'payment_mode' => 1]) }}" target="blank" class="inline-block w-full">
