<div class="flex flex-col max-h-[32rem]">
    ...
    <div class="flex-1 overflow-auto">
        <table class="border-separate w-full border-spacing-0 border border-black">
            ...
            <tbody>
                <tr>
                    <th colspan="5" class="border border-black text-left px-4 whitespace-nowrap">REVENUE</th>
                </tr>
                @foreach ($this->accounts->where('account_type_id', 3) as $account)
                ...
                <tr class="hover:bg-green-100">
                    ...
                    <td
                        ...
                        {{ renumber_format(sum_no_children_recursive($this->accounts->where('account_type_id', 3), 'ending_balance')) }}
                    </td>
                    <td
                        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
                        {{ renumber_format(sum_no_children_recursive($this->accounts->where('account_type_id', 3), 'balance_forwarded_credit')) }}
                    </td>
                    <td
                        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
                        {{ format_percentage(sum_no_children_recursive($this->accounts->where('account_type_id', 3), 'ending_balance'), sum_no_children_recursive($this->accounts->where('account_type_id', 3), 'balance_forwarded_credit')) }}
                    </td>
                    <td
                        class="border hover:bg-green-300 border-black px-2 uppercase text-xs text-right whitespace-nowrap">
                    </td>
