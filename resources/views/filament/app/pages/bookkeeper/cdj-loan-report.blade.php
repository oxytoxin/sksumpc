    ...
    <x-app.cashier.reports.report-layout>
        ...
        <div class="p-4 overflow-auto ">
            <table class="print:text-[10pt] w-full">
                <thead>
                    <tr>
                        <th rowspan="2" colspan="3" class="text-center border border-black whitespace-nowrap p-2">CDJ
                            SL- LOANS RECEIVABLES- {{ oxy_get_month_range()[$data['month']] }} {{ $data['year'] }}</th>
                        <th colspan="{{ $this->loan_types->count() * 2 + 2 }}"
                            class="text-center border border-black whitespace-nowrap p-1">LOANS</th>
                        <th colspan="{{ $this->loan_types->count() * 2 + 2 }}"
                            class="text-center border border-black whitespace-nowrap p-1">INTEREST</th>
                    </tr>
                    <tr>
                        @foreach ($this->loan_types as $loan_type)
                            <th colspan="2" class="text-center border border-black whitespace-nowrap px-4">
                                {{ $loan_type->code }}</th>
                        @endforeach
                ...
                <tbody>
                    ...
                    @forelse ($this->receivables as $receivable)
                        ...
                        @endphp
                        <tr>
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ $receivable->transaction_date->format('m/d/Y') }}</td>
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ $receivable->member->alt_full_name }}</td>
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ $receivable->reference_number }}</td>
                            @foreach ($this->loan_types as $loan_type)
                            ...
                            @endphp
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ number_format($total_loan_debit, 2) }}</td>
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ number_format($total_loan_credit, 2) }}</td>
                            @php
                            ...
                            <td class="text-left px-4 border border-black whitespace-nowrap">
                                {{ $total_interest_credit ? number_format($total_interest_credit, 2) : '' }}</td>
                            @php
