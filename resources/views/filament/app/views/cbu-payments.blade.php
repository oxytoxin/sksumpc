    ...
    <tbody>
        @forelse ($cbu->payments as $payment)
            <tr>
                ...
                <td class="doc-table-cell">{{ $payment->amount < 0 ? Filament\Support\format_money($payment->withdrawal ?? 0, 'PHP') : '' }}</td>
                <td class="doc-table-cell">{{ $payment->amount > 0 ? Filament\Support\format_money($payment->deposit ?? 0, 'PHP') : '' }}</td>
                <td class="doc-table-cell">{{ Filament\Support\format_money($payment->running_balance, 'PHP') }}
                </td>
