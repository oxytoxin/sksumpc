            ...
            <thead>
                <tr>
                    ...
                    @foreach ($loan_types as $loan_type)
                        <th colspan="3" class="text-center border-2 border-black whitespace-nowrap px-4">{{ $loan_type->code }}</th>
                    @endforeach
            ...
            <tbody>
                @foreach ($this->table->getRecords() as $loan_payment)
                    <tr>
                        ...
                        <td class="text-left px-4 border-2 border-black whitespace-nowrap">{{ $loan_payment->loan->member->alt_full_name }}</td>
                        <td class="text-right px-4 border-2 border-black whitespace-nowrap">{{ $loan_payment->reference_number }}</td>
                        @foreach ($loan_types as $loan_type)
                            <td class="text-right px-4 border-2 border-black whitespace-nowrap">{{ $loan_payment->loan->loan_type_id ==  $loan_type->id ? number_format($loan_payment->principal_payment, 2) : '' }}</td>
                            <td class="text-right px-4 border-2 border-black whitespace-nowrap">{{ $loan_payment->loan->loan_type_id ==  $loan_type->id ? number_format($loan_payment->interest, 2) : '' }}</td>
                            <td class="text-right px-4 border-2 border-black whitespace-nowrap">{{ $loan_payment->loan->loan_type_id ==  $loan_type->id ? '' : '' }}</td>
                        @endforeach
                        <td class="text-right px-4 border-2 border-black whitespace-nowrap font-bold">{{ number_format($loan_payment->amount, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
