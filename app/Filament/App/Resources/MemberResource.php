                    ...
                    Grid::make(3)
                        ->schema([
                            Select::make('civil_status_id')
                                ->label('Civil Status')
                                ->options(CivilStatus::pluck('name', 'id'))
                                ->default(1),
        ...
        return $table
            ...
            ->recordActions([
                ActionGroup::make([
                    ...
                    Action::make('closed_account')
                        ->label('Closed Account')
                        ->requiresConfirmation()
                        ->visible(Auth::user()->can('manage members'))
                        ->hidden(fn ($record) => $record->terminated_at)
                        ->modalHeading('Closed Account')
                        ->schema([
                            Placeholder::make('closure_disclosure')
                                ->label('Account Closure Disclosure')
                                ->content(fn (Member $record) => view('filament.app.components.account-closure-disclosure', [
                                    'disclosure' => app(BuildAccountClosureDisclosure::class)->handle($record),
                                ])),
                            ...
                            TextInput::make('capital_amount_closed')->label('Capital Amount Closed')->numeric()->prefix('₱')
                                ->default(fn (Member $record) => app(BuildAccountClosureDisclosure::class)->handle($record)['cbu'])
                                ->required(),
                        ])
                        ...
                        ->action(function ($record, $data) {
                            app(CloseMemberAccount::class)->handle(
                                member: $record,
                                bodResolution: $data['bod_resolution'],
                                terminationDate: $data['termination_date'],
                                voucherNumber: $data['termination_voucher_number'],
                                capitalAmount: (float) $data['capital_amount_closed'],
                            );
