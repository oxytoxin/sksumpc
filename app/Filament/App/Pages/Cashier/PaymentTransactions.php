                                ...
                                Section::make('')
                                    ...
                                    ->schema([
                                        Select::make('savings_account_id')
                                            ->options(fn ($get) => SavingsAccount::whereMemberId($get('../../../member_id'))->active()->pluck('number', 'id'))
                                            ->label('Account')
                                            ->required()
                                            ->suffixAction(
                            ...
                            ->schema([
                                Section::make('')
                                    ...
                                    ->schema([
                                        ...
                                        Placeholder::make('surcharge_due')
                                            ->content(function ($get) {
                                                $loan = LoanAccount::find($get('loan_account_id'))?->loan;

                                                return $loan
                                                    ? Number::currency(LoansProvider::computeSurchargeDue($loan, $this->transaction_date), 'PHP')
                                                    : null;
                                            }),
                                        Placeholder::make('total_settlement')
                                            ->content(function ($get) {
                                                $loan = LoanAccount::find($get('loan_account_id'))?->loan;

                                                return $loan
                                                    ? Number::currency(LoansProvider::computeSettlementAmount($loan, $this->transaction_date), 'PHP')
                                                    : null;
                                            }),
                                        Select::make('payment_type_id')
                                            ->paymenttype()
                ...
                Actions::make([
                    Action::make('submit')
                        ->action(function () {
                            ...
                            if ($form === null) {
                                throw new \LogicException('The cashier transaction form is unavailable.');
                            }
                            ...
                            if ($member?->terminated_at) {
                                throw ValidationException::withMessages([
                                    'member_id' => 'Closed accounts cannot accept cashier transactions.',
                                ]);
                            }
