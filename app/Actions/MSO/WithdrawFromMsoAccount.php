...
class WithdrawFromMsoAccount
{
    public function handle(MsoType $msoType, TransactionData $data)
    {
        ...
        if (($account?->isClosed() || $member?->terminated_at) && ! $data->allow_full_balance_withdrawal) {
            throw ValidationException::withMessages([
                'account_id' => 'Closed accounts cannot accept withdrawals.',
            ]);
        }
        ...
        if (! $data->allow_full_balance_withdrawal && $balance - $data->debit < 500) {
