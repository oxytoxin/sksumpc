...
class CreateNewSavingsTransaction
{
    use AsAction;
    ...
    public function handle(Member $member, SavingsData $data)
    {
        ...
        $member->savings_no_interest()->where('savings_account_id', $data->savings_account_id)->each(function ($saving) use ($data) {
            $saving->update([
                'interest' => $this->calculateInterest($saving->balance, $saving->interest_rate, Carbon::make($data->transaction_date)->diffInDays($saving->transaction_date)),
    ...
    protected function calculateInterest($amount, $interest, $days)
    {
