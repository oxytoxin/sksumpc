 ...
 * @property-read CarbonImmutable $asOfDate
 * @property-read int|null $selectedLoanTypeId
 * @property-read Collection<int, Loan> $loans
 * @property-read array<string, array{name: string, days: array{int, int}, loans: Collection<int, Loan>, total: float|int, count: int}> $agingBuckets
 */
class BookkeeperLoanAgingTab extends Component implements HasForms
{
    ...
    public function getLoansQuery(): Builder
    {
        $query = Loan::query()
            ->with(['member', 'loan_type'])
            ->wherePosted(true)
            ->where('outstanding_balance', '>', 0)
            ->whereDate('maturity_date', '<=', $this->asOfDate)
    ...
    public function getAgingBucketsProperty(): array
    {
        ...
        $buckets = [
            '0-30' => ['name' => 'Matured / 0-30 Days', 'days' => [0, 30], 'loans' => collect(), 'total' => 0, 'count' => 0],
    ...
    public function getLoanTypeLabelProperty(): string
    {
        ...
        $loanType = LoanType::query()->find($this->selectedLoanTypeId);

        return $loanType instanceof LoanType ? $loanType->name : 'Loans';
    }
