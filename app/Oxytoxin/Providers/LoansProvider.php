...
class LoansProvider
{
    ...
    const SURCHARGE_GRACE_WORKING_DAYS = 7;

    public static function calculateMaturityDate(Loan $loan): CarbonImmutable
    {
        $releaseDate = CarbonImmutable::parse($loan->release_date ?? $loan->transaction_date);

        if ($loan->loan_type_id === LoanTypes::SPECIAL_LOAN->value) {
            if ($releaseDate->isAfter(CarbonImmutable::create($releaseDate->year, 11, 20))) {
                return CarbonImmutable::create($releaseDate->year + 1, 5, 20);
            }

            if ($releaseDate->isAfter(CarbonImmutable::create($releaseDate->year, 5, 20))) {
                return CarbonImmutable::create($releaseDate->year, 11, 20);
            }

            return CarbonImmutable::create($releaseDate->year, 5, 20);
        }

        return CarbonImmutable::parse($loan->transaction_date)->addMonthsNoOverflow($loan->number_of_terms);
    }

    public static function getSurchargeStartDate(Loan $loan): CarbonImmutable
    {
        $date = $loan->maturity_date
            ? CarbonImmutable::parse($loan->maturity_date)
            : self::calculateMaturityDate($loan);
        $workingDays = 0;

        while ($workingDays < self::SURCHARGE_GRACE_WORKING_DAYS) {
            $date = $date->addDay();
        ...
        if ($loan->loan_application_id) {
            $configuredStartDate = $loan->loan_application->surcharge_start_date;

            if ($configuredStartDate?->isAfter($date)) {
                return CarbonImmutable::parse($configuredStartDate);
            }
        }
    ...
    public static function computeSurchargeDue(Loan $loan, CarbonInterface|string $transactionDate): float
    {
        $transactionDate = CarbonImmutable::parse($transactionDate);

        if (
            $loan->loan_type_id !== LoanTypes::SPECIAL_LOAN->value
            || $transactionDate->isBefore(self::getSurchargeStartDate($loan))
            || (float) $loan->outstanding_balance <= 0
        ) {
            return 0;
        }

        $surchargeRate = (float) $loan->loan_type->surcharge_rate;
        $assessedSurcharge = round((float) $loan->outstanding_balance * $surchargeRate, 2);
        $paidSurcharge = $loan->exists
            ? (float) $loan->payments()
                ->whereDate('transaction_date', '<=', $transactionDate)
                ->sum('surcharge_payment')
            : 0;

        return max(round($assessedSurcharge - $paidSurcharge, 2), 0);
    }

    public static function computeInterestDue(Loan $loan, CarbonInterface|string $transactionDate): float
    {
        $transactionDate = CarbonImmutable::parse($transactionDate);
        $lastPayment = $loan->payments()
            ...
            ->latest('id')
            ->first();
        $start = $lastPayment instanceof LoanPayment
            ? $lastPayment->transaction_date
            : $loan->transaction_date;
        $totalDays = self::getAccruableDays($start, $transactionDate);
        $unpaidInterest = (float) $loan->payments()
            ->whereDate('transaction_date', '<=', $transactionDate)
            ->sum('unpaid_interest');

        return max(round(self::computeAccruedInterest($loan, $loan->outstanding_balance, $totalDays) + $unpaidInterest, 2), 0);
    }

    public static function computeSettlementAmount(Loan $loan, CarbonInterface|string $transactionDate): float
    {
        return round(
            (float) $loan->outstanding_balance
            + self::computeSurchargeDue($loan, $transactionDate)
            + self::computeInterestDue($loan, $transactionDate),
            2,
        );
    }
    ...
    {
        $days = LoansProvider::getAccruableDays($start_date, $end_date);

        return LoansProvider::computeAccruedInterest($loan, $outstanding_balance, $days);
    }
